...
7. **TODO — Regenerate `site/` (stellardeck.dev) with three step-by-step "Get started" tracks: (a) Desktop app (download .dmg, drop a .md, you're presenting), (b) CLI (`npm i -g stellardeck` once published, `stellardeck export deck.md`), (c) VS Code extension (Marketplace install, `Cmd+K V` on any .md). Right after the npm publish lands so the install snippets actually work.** Today the site is Starlight in `site/` but the install paths it documents pre-date the package extraction + the VS Code ext. Each track should land on a 30-second "first success" before the docs deepen — three concrete journeys, not a feature list. Asked by Paulo 2026-05-06.
